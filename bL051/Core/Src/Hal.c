...
HAL_StatusTypeDef halHAL_Init(void)
{
  ...
  if (halHAL_InitTick(TICK_INT_PRIORITY) != HAL_OK)
  {
    status = HAL_ERROR;
  }
  else
...
/**
  ...
  * @retval HAL status
  */
__weak HAL_StatusTypeDef halHAL_InitTick(uint32_t TickPriority)
{
  /* Configure the SysTick to have interrupt in 1ms time basis*/
  if (corHAL_SYSTICK_Config(SystemCoreClock / (1000U / uwTickFreq)) > 0U)
  {
    return HAL_ERROR;
  }
  ...
  {
    corHAL_NVIC_SetPriority(SysTick_IRQn, TickPriority, 0U);
 ...
 * @note This function is declared as __weak to be overwritten in case of other
  ...
  * @retval None
  */
__weak void halHAL_IncTick(void)
{
  uwTick += uwTickFreq;
}
...
/**
  ...
  * @retval tick value
  */
__weak uint32_t halHAL_GetTick(void)
{
  return uwTick;
}
#endif
